...
const runDiagnostics = async () => {
  ...
  try {
    const { error } = await supabase.from('registration_table').select('count', { count: 'exact', head: true });
    if (error && (error.message?.includes("fetch") || error.message?.includes("network"))) throw new Error("Network error: " + error.message);
  ...
  try {
    const { data, error } = await supabase.from('registration_table').select('*').limit(1);
    if (error) throw new Error(`${error.code} — ${error.message}`);
    console.log("✅ Step 3: registration_table accessible. Rows:", data.length);
  } catch (err) {
    console.error("❌ Step 3 FAILED:", err.message);
    console.groupEnd(); return;
  }
  try {
    const testPayload = { name: '__diag_test__', email: '__diag@test.com', phone: '0000000000', age: 25, password: 'test', profession: 'test' };
    const { data: inserted, error: insertError } = await supabase.from('registration_table').insert([testPayload]).select().single();
    if (insertError) {
      if (insertError.code === '23505') { console.log("✅ Step 4: INSERT works (duplicate key)"); }
      else throw new Error(`INSERT failed: ${insertError.code} — ${insertError.message}`);
    } else {
      console.log("✅ Step 4: INSERT works:", inserted);
      await supabase.from('registration_table').delete().eq('email', '__diag@test.com');
...
export default function AuthPage({ onLoginSuccess }) {
  ...
  const handleLoginSubmit = async (e) => {
    ...
    try {
      ...
      const { data: user, error } = await supabase.from('registration_table').select('*').eq(column, identifier).single();
      ...
      if (user.password !== loginData.password) { alert("❌ Wrong password. Please try again."); setIsLoggingIn(false); return; }
      ...
      try {
        ...
        if (err.code === 1) {
          alert(
            "📍 Location access is required to log in.\n\n" +
            "To enable it:\n" +
          ...
          setIsLoggingIn(false);
          return; // ← stops here, user tries again themselves
        }
        console.warn("Location unavailable:", err.message);
        // timeout/unavailable — proceed without location
      }

      // Insert into login_log
      const { data: logRow, error: logError } = await supabase
        .from('login_log')
        .insert({ user_id: user.id, ip: ipAddress, lat, long })
        .select()
        .single();

      if (logError) console.warn("Login log insert failed:", logError.message);

      alert(`✅ Logged in successfully! Welcome back, ${user.name} 👋`);
      onLoginSuccess({ name: user.name, email: user.email, logId: logRow?.id });
  ...
  const handleSignupSubmit = async (e) => {
    ...
    const { data: existingEmail } = await supabase.from('registration_table').select('id').eq('email', signupData.email).single();
    if (existingEmail) { alert("❌ This email is already registered. Please log in."); setActiveTab('login'); return; }

    const { data: existingPhone } = await supabase.from('registration_table').select('id').eq('phone', signupData.phone).single();
    if (existingPhone) { alert("❌ This phone number is already registered. Please log in."); setActiveTab('login'); return; }

    const finalProfession = profession === 'Other' ? otherProfession : profession;
    const payload = { name: signupData.name, email: signupData.email, phone: signupData.phone, age: parsedAge, password: signupData.password, profession: finalProfession };

    try {
      const { data, error } = await supabase.from('registration_table').insert([payload]).select().single();
      if (error) {
        if (error.message?.includes("permission denied")) alert("🔴 [RLS Error] INSERT is blocked.");
        else if (error.code === '23505') { alert("❌ Account already exists. Please log in."); setActiveTab('login'); }
        else if (error.code === '23502') alert(`❌ Missing required field. Details: ${error.message}`);
